...
def genComp(name)
    auto suf = (name == "main") ? "" : ("_" + name)
                ...
                |->$LD $LFLAGS -Map=`name`.map -T linkcmd`suf`.ld -o $DIR/`name`.out $DIR/`name`.obj $LOBJS $LIBC $LIBGCC
...
def genScript(sysFlag)
    ...
    case "arm"
                ...
                |->LIBGCC=$TOOLS/lib/gcc/`gccFlav`/`gccVers`/thumb/v6-m/nofp/libgcc.a
    case "riscv32"
